...
個人規模の公開を想定した **構成案** です。学習用リポジトリには入れず、この本番フォークと `infra/` で管理します。

> **ポートフォリオ上の扱い:** 実サービスを AWS で公開しているわけではありません。`infra/` は設計用スケルトンで、**本プロジェクトでは `terraform apply` 未実施**です。動作デモはローカル Compose を使ってください（[README の Demo](../README.md#demo-local--preferred)）。
